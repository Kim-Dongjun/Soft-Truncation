...
def unpickle(file, data_dim=32):
  with open(file, 'rb') as fo:
    dict = np.load(fo)
    data = dict['data']
  data = np.dstack((data[:, :data_dim**2], data[:, data_dim**2:2*data_dim**2], data[:, 2*data_dim**2:]))
  data = data.reshape((data.shape[0], data_dim, data_dim, 3))
  return data
...
def get_batch(config, batch, scaler, dequantization=False, train='True'):
